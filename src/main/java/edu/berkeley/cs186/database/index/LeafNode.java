...
class LeafNode extends BPlusNode {
  ...
  private void keyRidComplete(DataBox key, RecordId rid) {
    keys.add(key);
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
    ...
    if (keys.contains(key)) {
      ...
      if (keys.size() <= capacity) {
        ...
        LeafNode rsibling = new LeafNode(metadata, rKeys, rRids, rightSibling);

        DataBox pushKey = keys.get(d);

        keys = keys.subList(0, d);
        rids = rids.subList(0, d);
        int rightPgNum = rsibling.getPage().getPageNum();

        rightSibling = Optional.of(rightPgNum);
        sync();
        return Optional.of(new Pair<>(pushKey, rightPgNum));
      }
    }
  }
  ...
  public Optional<Pair<DataBox, Integer>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
                                                   float fillFactor)
      throws BPlusTreeException {
    int d = 2 * metadata.getOrder(); //maxOrder(getPage().getPageNum(), metadata.getKeySchema());
    int fill = (int) Math.ceil(d * fillFactor);
    while (data.hasNext()) {
      ...
      sync();
      if (keys.size() > fill) {
        List<DataBox> rKeys = keys.subList(fill, keys.size());
        List<RecordId> rRids = rids.subList(fill, rids.size());
        keys = keys.subList(0, fill);
        rids = rids.subList(0, fill);


        LeafNode leaf = new LeafNode(metadata, rKeys, rRids, rightSibling);

        DataBox pushKey = rKeys.get(0);
        int rightPgNum = leaf.getPage().getPageNum();
        rightSibling = Optional.of(rightPgNum);
        sync();
  ...
  public void remove(DataBox key) {
    int removeHere = keys.indexOf(key);
    if (removeHere != -1) {
      keys.remove(removeHere);
      rids.remove(removeHere);
      sync();
    }
  }
  ...
  public static LeafNode fromBytes(BPlusTreeMetadata metadata, int pageNum) {
    ...
    //Optional <Integer> rightSibling = (rightSib == -1) ? Optional.empty() : Optional.of(rightSib);
    int n = buf.getInt();
    for (int i = 0; i < n; i++) {
