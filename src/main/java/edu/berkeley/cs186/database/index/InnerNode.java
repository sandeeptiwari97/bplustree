...
class InnerNode extends BPlusNode {
  ...
  public LeafNode get(DataBox key) {
    int getThis = numLessThanEqual(key, keys);
    return getChild(getThis).get(key);
  }
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
    ...
    int cIndex = numLessThanEqual(key, keys);
    int d = metadata.getOrder();

    BPlusNode child = getChild(cIndex);
    Optional<Pair<DataBox, Integer>> returned = child.put(key, rid);
    if (!returned.isPresent()) {
      sync();
      return Optional.empty();//returned;
    } else {
      DataBox returnKey = returned.get().getFirst();
      Integer returnPageNum = returned.get().getSecond();
      int index = numLessThan(returnKey, keys);
      keys.add(index, returnKey);
      children.add(index+1, returnPageNum);
      sync();
      if (keys.size() > 2*d) {
        DataBox pushKey = keys.get(d);
        List<DataBox> rKeys = keys.subList(d + 1, keys.size());
        List<Integer> rChildren = children.subList(d + 1, children.size());

        keys = keys.subList(0, d);//lKeys;
        children = children.subList(0, d + 1);//lChildren;

        InnerNode rightSib = new InnerNode(metadata, rKeys, rChildren);
        Integer rightPageNum = rightSib.getPage().getPageNum();
        sync();
        return Optional.of(new Pair<>(pushKey, rightPageNum));
      }
    }
    sync();
  ...
  public Optional<Pair<DataBox, Integer>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
    ...
    while (data.hasNext()) {
      ...
      if (!ret.equals(Optional.empty())) {
        DataBox retKey = ret.get().getFirst();
        Integer retPageNum = ret.get().getSecond();

        int d = metadata.getOrder();

        keys.add(retKey);
        children.add(retPageNum);
        sync();
        ...
        if (keys.size() > 2 * d) {
          ...
          keys = keys.subList(0, d);//lKeys;
          children = children.subList(0, d);//lChildren;
