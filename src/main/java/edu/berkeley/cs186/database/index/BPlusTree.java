...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
      typecheck(key);
      LeafNode getThis = root.get(key);
      return getThis.getKey(key);
    }
...
//      throw new UnsupportedOperationException("TODO(hw2): implement.");
    ...
    public void put(DataBox key, RecordId rid) throws BPlusTreeException {
      ...
      if (returned.isPresent()) {
          DataBox returnKey = returned.get().getFirst();
          Integer returnPgNum = returned.get().getSecond();
          Integer currPage = root.getPage().getPageNum();
          List<Integer> children = new ArrayList<Integer>(Arrays.asList(currPage, returnPgNum));
          List<DataBox> keys = new ArrayList<DataBox>(Arrays.asList(returnKey));
          InnerNode newRoot = new InnerNode(metadata, keys, children);
//          headerPage = newRoot.getPage();
          root = newRoot;
          writeHeader(headerPage.getByteBuffer());
      }
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) throws BPlusTreeException {
        ...
        if (((LeafNode) root).scanAll().hasNext()) {
            throw new BPlusTreeException();
        }
        Optional<Pair<DataBox, Integer>> ret = root.bulkLoad(data, fillFactor);
        if (!ret.equals(Optional.empty())) {
            DataBox key = ret.get().getFirst();
            Integer rightPgNum = ret.get().getSecond();
            ArrayList<DataBox> keys = new ArrayList<>();
            keys.add(key);

            ArrayList<Integer> children = new ArrayList<>();
            children.add(root.getPage().getPageNum());
            children.add(rightPgNum);

            InnerNode split = new InnerNode(metadata, keys, children);
            root = split;
            split.bulkLoad(data, fillFactor);
            writeHeader(headerPage.getByteBuffer());
        }
    }
...
//      throw new UnsupportedOperationException("TODO(hw2): implement.");
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
      ...
      private Iterator<RecordId> bplusiterator;
      private LeafNode currLeaf;
      public BPlusTreeIterator(BPlusNode root)  {
        currLeaf = root.getLeftmostLeaf();
        bplusiterator = currLeaf.scanAll();
//        while (!currLeaf.equals(Optional.empty())) {
//            leaf.clear();
//            currLeaf.scanAll().forEachRemaining(leaf::add);
//            allRids.addAll(leaf);
//            currLeaf = currLeaf.getRightSibling().get();
//
//        }
//        bplusiterator = allRids.iterator();
      }

      public BPlusTreeIterator(BPlusNode root, DataBox key) {
          currLeaf = root.get(key);
          bplusiterator = currLeaf.scanGreaterEqual(key);
      }

      @Override
      public boolean hasNext() {
          if (bplusiterator.hasNext()) {
              return true;
          } else if (!currLeaf.getRightSibling().equals(Optional.empty())){
              currLeaf = currLeaf.getRightSibling().get();
              bplusiterator = currLeaf.scanAll();//scanEqual(currLeaf.getKeys().get(0));
              return bplusiterator.hasNext();
          }
          return false;
      }
      @Override
      public RecordId next() {
          ...
          if (!bplusiterator.hasNext() && !currLeaf.getRightSibling().equals(Optional.empty())) {
              currLeaf = currLeaf.getRightSibling().get();
              bplusiterator = currLeaf.scanAll();
          }
          return bplusiterator.next();
      }
    }
}
